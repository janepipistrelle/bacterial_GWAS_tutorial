...
Dependencies
-------------------------
The code we are using today is taken from our GWAS pipeline, which can be found here: https://github.com/jessiewu/bacterialGWAS

Our pipeline is written in R so we are going to use R on the command line to run some of the scripts from the pipeline. It relies on some external dependencies, which are `BLAST` and ```Gemma``` (Gemma version 0.93 needed for _bugwas_) and the installation of the R package _bugwas_. For this example we are only doing a kmer GWAS. The dataset we will use is a subset of genomes from a study aimed at detecting antibiotic resistance variants in _S. aureus_ (Gordon et al 2013).
