...
-------------------------
1. Launch EC2 instance: we will run Ubuntu 14.04 LTS (64-bit) on an m3.2xlarge instance. Before you click 'Review and Launch', increase the root file system size to 30 GB.
...
git clone https://github.com/xiangzhou/GEMMA.git
```
...
Finally, let's open R and install a couple of packages.
```
R
install.packages("genoPlotR")
install.packages("\soft/bugwas/bugwas_1.0.tar.gz", repos = NULL, type="source")
```

Then use your favourite text editer to create a list of the paths to each of the following dependencies in the following tab-delimited format:
name  path
GEMMA  ~/soft/gemma
blastn ~/soft/blastn
...
```
#Bash Shell
Rscript kmerAnalysis.R –dataFile fus300.kmerfiles.txt –prefix fus –removeKmerTxt FALSE –mincov 5 –externalSoftware dependencies.txt
```
...
```
#Bash Shell
Rscript kmerAnnotation.R –chisq_results fus300.gwaskmer-out.chisqStat.txt –kmer_results fus300.gwaskmer-out.kmer.txt –present_ctrl fus300.gwaskmer-out.nPresentCtrl.txt  –present_case  fus300.gwaskmer-out.nPresentCase.txt –blastdb1 ~/data/dbs/staphdb3.blast –blastdb2 ~/data/dbs/nt_blastdb –ncbi_db ~/data/dbs/geneannot.allstaph.txt –signif 5000 –nproc 2 –prefix fus300 –externalSoftware dependencies.txt
```
...
lin_loc(gen="~/data/fus300_bugwas_gemma_gen_format.txt",pheno="~/data/fus300_bugwas.pheno.txt",phylo="~/data/RAxML_bestTree.fus300",prefix="fus300",gem.path="~/soft/bugwas-master/gemma/gemma.0.93b",var.matrix="~/data/fus300_bugwas.var.matrix.txt",relmatrix="~/data/fus300_gemma_relmatrixout.cXX.txt", output.dir=output.dir)
