...
tar -xvf jane-data.tar.gz jane-data2.tar.gz
```
...
Then use your favourite text editer to create a list of the paths to each of the following dependencies in the following tab-delimited format, calling it ```dependencies.txt```.
...
```
#Bash Shell
Rscript kmerAnalysis.R –dataFile ~/data/fus300.kmerfiles.txt –prefix fus –removeKmerTxt FALSE –mincov 5 –externalSoftware dependencies.txt
```
